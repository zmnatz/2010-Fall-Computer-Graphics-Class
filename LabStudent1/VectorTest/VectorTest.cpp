...
int main(int argc, char* argv[])
{
   ...
   logmsg("Reflection of v off the x-axis is (%.2f %.2f)", r.x, r.y);
   ...
   // Find the distance of point c from line segment ab
   Vector3 proj = Vector3(a3,c3).Projection(Vector3(a3,b3));
		float projNormSqrd = proj.NormSquared();
		float abNormSqrd = Vector3(a3,b3).NormSquared();
		float angleBetween = radiansToDegrees(proj.AngleBetween(Vector3(a3,b3)));

		logmsg("%.2f %.2f %.2f",angleBetween,projNormSqrd,abNormSqrd);
   Point3 closestPt3;
   ...
   plane.Normalize();
   logmsg("Normalized Plane = %.2f %.2f %.2f %.2f", plane.a, plane.b, plane.b, plane.d);
